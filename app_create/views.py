...
def selenium_search_ozon_monitor(request):
    ...
    for i in items:
        ...
        for k in items:
            ...
            string=str(string[1])
            string=string.split(',')
            model_card_string=str(string[0])
            ...
            #если в словаре есть значение цвета, добавляем его название в Название товара
            #цвет необязательный атрибут
            #transforming list to string
            type_brand_string=' '.join(type_brand_string)
            if specs['Цвет товара']:
                string=[
                    type_brand_string,
                    str(specs['Цвет товара']).lower()
                    ]
            else:
                try:
                    string=string[1]
                    specs['Цвет товара'] = string.lower()
                    string=[
                        type_brand_string,
                        specs['Цвет товара']
                    ]
